...
Runs all modules and generates heatmap + analytics report.
"""
...
def run_full_pipeline(config: dict) -> None:
    ...
    output_path = "data/outputs/full_pipeline_output.mp4"
    ...
    logger.info(f"Running full pipeline on {total_frames} frames")

    frame_count = 0
    last_frame = None

    with tqdm(total=total_frames, desc="Full pipeline") as pbar:
        while True:
            ret, frame = cap.read()
            if not ret:
                break
    ...
    # Save analytics
    analytics.save_csv("data/outputs/analytics.csv")
    analytics.save_json("data/outputs/analytics.json")
    analytics.print_summary()

    logger.info(f"Pipeline output : {output_path}")
    logger.info(f"Heatmap         : {heatmap_path}")
    logger.info(f"Analytics CSV   : data/outputs/analytics.csv")
    logger.info(f"Analytics JSON  : data/outputs/analytics.json")
